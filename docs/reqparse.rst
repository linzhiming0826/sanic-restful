...
.. warning ::
    ...
    The whole request parser part of Sanic-RESTful is slated for removal and
...
Sanic-RESTful's request parsing interface, :mod:`reqparse`, is modeled after
...
:class:`sanic.request` object in Sanic.

Basic Arguments
---------------

Here's a simple example of the request parser. It looks for two arguments in
the :attr:`sanic.Request.values` dict: an integer and a string ::
...
:attr:`sanic.Request.values`, and :attr:`sanic.Request.json`.
...
:class:`sanic.Request` can be used. For example: ::
...
If the argument location list includes the :attr:`~sanic.Request.headers`
...
specified either at the Sanic application level or on the specific
